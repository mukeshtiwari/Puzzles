...
By Fermat's little theorem, g^q = g^(rq) = g^(p-1) = 1
-/
...
theorem elgamal_enc_dec_identity : ∀ m r c, c = elgamal_enc p q g pubkey m r →
...
theorem additive_homomorphic_property : ∀ c d m₁ m₂ r₁ r₂, c = elgamal_enc p q g pubkey m₁ r₁ ->
 d = elgamal_enc p q g pubkey m₂ r₂ ->  (g ^ (r₁.val + r₂.val), g ^ (m₁.val + m₂.val) * 
...
/- zero knowledge proof of that a ciphertext is 
   is encryption of 0 or 1 -/
...
#eval elgamal_enc 983 491 158 905 10 20
#eval elgamal_dec 983 491 331 (248, 592) -- evaluated g^m (see below)
#eval (158^10)%983
